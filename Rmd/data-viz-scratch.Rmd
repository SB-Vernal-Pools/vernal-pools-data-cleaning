...
## ===========================================
# read in data 
hydro <- read_csv(here("data", "hydro_corrected_new.csv")) %>% 
  clean_names()
...
hydro_clean <- hydro %>% 
  ...
  # convert date to date-time object (VS)
  mutate(date = mdy_hms(date),
         date = as.Date(date),
         
         # create unique pool_id identifier
         location_pool_id = paste0(location, "_", pool_id)) %>%
  
  # calculate water year using custom function (VS)
...
species_abdundance <- percent_cover %>% 
  ...
  mutate(pool_id = paste0(location, "_", pool_id),
         ...
         # rename type for cleaner labels (VS)
...
# remove underscores from species names (VS)
...
percent_cover_clean <- percent_cover %>% clean_names() %>%
  ...
  filter(pool_id == "ncos_1" & transect_axis == "minor")
