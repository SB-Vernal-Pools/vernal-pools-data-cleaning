...
species_abdundance <- percent_cover %>% 
  ...
  # filter for app (RJ)
  filter(location_pool_id == "West Campus - 4" & complete.cases(species) & species != "unlisted")

# percent cover (quadrat level)
percent_cover_clean <- percent_cover %>% clean_names() %>%
  
  # filter for app (RJ)
  filter(location_pool_id == "North Campus Open Space - 1" & transect_axis == "minor")

# individual plant cover (quadrat level)
plant_subset <- percent_cover_clean %>% filter(species == "Eleocharis macrostachya") #filter for app (RJ)
...
## Plant Species Cover
```{r}
## NOTE: Updated geom_smooth() for all plots (RJ)
## ===========================================
##            Plant Species Abundance     ----
## ===========================================
