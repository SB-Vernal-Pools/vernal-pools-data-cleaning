...
hydro_data <- read_csv(here("data", "hydro_corrected.csv")) %>% 
  ...
  dplyr::select(-creator,-edit_date,-editor, -creation_date) %>% 
  mutate(location_pool_id = paste(location, pool_id, sep = "_"))
...
pc_abiotic_data <- read_csv(here("data", "pc_abiotic_df.csv")) %>% 
  clean_names() %>% 
  select(-global_id, -creator, -edit_date, creation_date, -creator, -editor) %>% 
  mutate(location_pool_id = paste(location, pool_id, sep = "_"))
...
hydro_abiotic_join <- full_join(hydro_data, pc_abiotic_data, by = "location_pool_id")


# TO DO : combine abiotic  and shape files ?
# hydro_vernal_pools_join <- full_join(hydro_data, vernal_pools, by = "location_pool_id")
